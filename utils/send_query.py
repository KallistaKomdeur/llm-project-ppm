...
class LLMSession:
    ...
    def send_query(self, provider: str, model_name: str, prompt: str) -> str:
    ...
    def _send_gemini(self, model_name: str, prompt: str, api_key: str) -> str:
        ...
        # Try to create/refresh cache (only works above minimum token threshold)
        try:
            if self._cache:
                self._client.caches.delete(name=self._cache.name)
            ...
            # Content too short to cache — silently fall back to passing history directly
            self._cache = None

        return output

    def _send_openai(self, model_name: str, prompt: str, api_key: str) -> str:
    ...
    def _send_anthropic(self, model_name: str, prompt: str, api_key: str) -> str:
